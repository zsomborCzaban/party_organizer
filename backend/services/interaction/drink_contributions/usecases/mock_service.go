...
func (m *MockService) Create(contribution domains.DrinkContribution, userId uint) api.IResponse {
	args := m.Called(contribution, userId)
	return args.Get(0).(api.IResponse)
}

func (m *MockService) Update(contribution domains.DrinkContribution, userId uint) api.IResponse {
	args := m.Called(contribution, userId)
	return args.Get(0).(api.IResponse)
}
