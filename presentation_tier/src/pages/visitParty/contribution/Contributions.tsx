...
const Contributions = () => {
    ...
    const makeOptions = (requirements: Requirement[]) => requirements.reduce<{ value: number, label: string }[]>((opts, requirement) => {
                  ...
                  <div style={styles.collapsible} key={req.ID}>
                    <Collapsible trigger={`${req.type} ${Math.round((dReqContributionMap[req.ID || -1] || 0) *100) / 100}/${req.target_quantity} ${req.quantity_mark}`} key={req.ID}>
                      {dContributions
                                            ...
                                            .map((contribution) => createContributionDiv(req, contribution, 'drink'))
                                        }
                    </Collapsible>
                  </div>
                ...
                {fRequirements.map(req => (
                  <div style={styles.collapsible} key={req.ID}>
                    <Collapsible trigger={`${req.type} ${Math.round((fReqContributionMap[req.ID || -1] || 0) *100) / 100}/${req.target_quantity} ${req.quantity_mark}`} key={req.ID}>
                      {fContributions
                                            .filter(con => con.requirement_id === req.ID)
                                            .map((contribution) => createContributionDiv(req, contribution, 'food'))
                                        }
                    </Collapsible>
                  </div>
                            ))}
