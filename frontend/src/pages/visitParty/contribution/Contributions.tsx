...
const Contributions = () => {
  ...
  const { requirements: dRequirements } = useSelector((state: RootState) => state.drinkRequirementStore);
  const { requirements: fRequirements } = useSelector((state: RootState) => state.foodRequirementStore);
  const { contributions: dContributions } = useSelector((state: RootState) => state.drinkContributionStore);
  const { contributions: fContributions } = useSelector((state: RootState) => state.foodContributionStore);
  const { participants } = useSelector((state: RootState) => state.partyParticipantStore);
  ...
  const createContributionDiv = (req: Requirement, contribution: Contribution, mode: string) => {
    ...
    return (
      ...
      >
        <div>
          {contributorName}: {contribution.quantity} {req.quantity_mark}, {contribution.description}
        </div>
  ...
  const makeOptions = (requirements: Requirement[]) =>
    requirements.reduce<{ value: number; label: string }[]>((opts, requirement) => {
  ...
  return (
    <div style={styles.background}>
      <div style={styles.outerContainer}>
        ...
        <ContributeModal
          mode='drink'
          options={makeOptions(dRequirements)}
        ...
        <ContributeModal
          mode='food'
          options={makeOptions(fRequirements)}
        ...
        <DeleteContributionModal
          visible={deleteContributionModalVisible}
          onClose={() => setDeleteContributionModalVisible(false)}
          mode={deleteMode}
        ...
        <div style={styles.container}>
          <div style={styles.outerCollapsible}>
            <Collapsible
              trigger={`Drinks ${fulfilledDReqs}/${dRequirements.length} fulfilled`}
              key='drinks'
            >
              <button
                style={styles.button}
                onClick={() => {
                  handleContribute('drink');
                }}
              >
                Contribute
              </button>
              {dRequirements.map((req) => (
                ...
                >
                  <Collapsible
                    trigger={`${req.type} ${Math.round((dReqContributionMap[req.ID || -1] || 0) * 100) / 100}/${req.target_quantity} ${req.quantity_mark}`}
                    key={req.ID}
                  >
                    {dContributions.filter((con) => con.requirement_id === req.ID).map((contribution) => createContributionDiv(req, contribution, 'drink'))}
                  </Collapsible>
                </div>
              ))}
            </Collapsible>
          ...
          <div style={styles.outerCollapsible}>
            <Collapsible trigger={`Foods ${fulfilledFReqs}/${fRequirements.length} fulfilled`}>
              ...
              {fRequirements.map((req) => (
                ...
                >
                  <Collapsible
                    trigger={`${req.type} ${Math.round((fReqContributionMap[req.ID || -1] || 0) * 100) / 100}/${req.target_quantity} ${req.quantity_mark}`}
                    key={req.ID}
                  >
                    {fContributions.filter((con) => con.requirement_id === req.ID).map((contribution) => createContributionDiv(req, contribution, 'food'))}
                  </Collapsible>
                </div>
              ))}
            </Collapsible>
